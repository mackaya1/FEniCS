...
w_j =  50.0                     # Characteristic Rotation Speed (rotations/s)
lambda1=1.0*r_b/(r_a*w_j)         # Relaxation Time
...
while j < loopend:
    ...
    mu_1 = 25.0*(3.0)*10E-3             # Solvent Viscosity 
    mu_2 = 25.0*(3.0)*10E-3             # Polymeric Viscosity
    ...
    alpha=1.0/(rho_0*Cv)                # Thermal Diffusivity
    Bi=0.0075                           # Biot Number
    ...
    #Half Step
    a1=(1.0/(dt/2.0))*inner(Ret*rho0*u,v)*dx+betav*(inner(grad(u),grad(v))*dx+1.0/3*inner(div(u),div(v))*dx)#\
         #+ theta*(inner(grad(u),grad(v))*dx+1.0/3*inner(div(u),div(v))*dx)
    L1=(1.0/(dt/2.0))*inner(Ret*rho0*u0,v)*dx-inner(Ret*rho0*grad(u0)*u0,v)*dx+inner(p0,div(v))*dx \
        -inner(tau0,grad(v))*dx #+ theta*inner(D,grad(v))*dx

    #Predicted U* Equation
    a2=(1.0/dt)*inner(Ret*rho0*u,v)*dx #+ theta*(inner(grad(u),grad(v))*dx+1.0/3*inner(div(u),div(v))*dx)
    L2=(1.0/dt)*inner(Ret*rho0*u0,v)*dx-0.5*betav*(inner(grad(u0),grad(v))*dx+1.0/3*inner(div(u0),div(v))*dx) \
        -inner(Ret*rho0*grad(u12)*u12,v)*dx+inner(p0,div(v))*dx-inner(tau0,grad(v))*dx #\
    ...
    a5=(1.0/dt)*inner(p,q)*dx+0.5*dt*inner(c0c0*grad(p),grad(q))*dx   #Using Dynamic Speed of Sound (c=c(x,t))
    ...
    #Velocity Update
    a7=(1.0/dt)*inner(Ret*rho0*u,v)*dx+0.5*betav*(inner(grad(u),grad(v))*dx+1.0/3*inner(div(u),div(v))*dx)#\
         #+ theta*(inner(grad(u),grad(v))*dx+1.0/3*inner(div(u),div(v))*dx)
    L7=(1.0/dt)*inner(Ret*rho0*us,v)*dx+0.5*(inner(p1,div(v))*dx-inner(p0,div(v))*dx) #+ theta*inner(D,grad(v))*dx
    ...
    L8 = (2.0/dt)*inner(rho1*thetar,r)*dx + Di*inner(grad(thetar),grad(r))*dx + inner(rho1*dot(u12,grad(thetar)),r)*dx \
          + (2.0/dt)*inner(rho1*theta0,r)*dx + Vh*(inner(gamdots,r)*dx + inner(gamdotp,r)*dx) - Di*Bi*inner(theta0,r)*ds(1) + thetat*(inner(grad(thetar),grad(r))*dx+inner(Dt,grad(r))*dx)
    ...
    L9 = (1.0/dt)*inner(rho1*thetar,r)*dx + Di*inner(grad(thetar),grad(r))*dx + inner(rho1*dot(u1,grad(thetar)),r)*dx \
          + (1.0/dt)*inner(rho1*theta0,r)*dx + Vh*(inner(gamdots12,r)*dx + inner(gamdotp12,r)*dx) - Di*Bi*inner(theta0,r)*ds(1) + thetat*(inner(grad(thetar),grad(r))*dx+inner(Dt,grad(r))*dx)
          #+ inner(,r)*dx  #Neumann Condition on the outer bearing is encoded in the weak formulation
    ...
    while t < Tf + DOLFIN_EPS and iter < maxiter:
        ...
        if iter > 1:
           xx.append(t)
           yy.append(norm(h,'linf')/norm(p1.vector()))
           zz.append(norm(o,'linf')/(norm(tau1.vector())+0.0001))
    ...
    plt.xlabel('time(s)')
    plt.ylabel('Torque')
    plt.savefig("Compressible Viscoelastic Flow Results/Stability-Convergence/Torque We="+str(We)+"Re="+str(Rey)+"t="+str(t)+".png")
    plt.clf()
    plt.plot(x, z, 'b-', label='Vertical Force')
    plt.xlabel('time(s)')
    plt.ylabel('Vertical Force')
    plt.savefig("Compressible Viscoelastic Flow Results/Stability-Convergence/Vertical Load We="+str(We)+"Re="+str(Rey)+"t="+str(t)+".png")
    plt.clf()


    if max(norm(T1.vector(),'linf'),norm(p1.vector(), 'linf'),norm(u1.vector(), 'linf')) < 10E5:

        # Matlab Plot of the Solution at t=Tf
        rho1=rho_0*rho1
        rho1=project(rho1,Q)
        p1=mu_0*(c/(w_j*r_a))*p1  #Dimensionalised Pressure
        p1=project(p1,Q)
        mplot(rho1)
        plt.colorbar()
        ...
        plt.savefig("Compressible Viscoelastic Flow Results/Plots-Contours/Pressure Contours We="+str(We)+"Re="+str(Rey)+"t="+str(t)+".png")
        plt.clf()
        ...
        plt.savefig("Compressible Viscoelastic Flow Results/Plots-Contours/Temperature Contours We="+str(We)+"Re="+str(Rey)+"t="+str(t)+".png")
        plt.clf()
        ...
        plt.savefig("Compressible Viscoelastic Flow Results/Plots-Contours/Velocity Contours We="+str(We)+"Re="+str(Rey)+"t="+str(t)+".png")   
