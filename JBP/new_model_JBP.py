...
while j < loopend:
    ...
    restau0 = We/dt*(tau1_vec-tau0_vec) + We*F1R_vec + fene_func(tau0, b)*tau1_vec - I_vec
    ...
    k_a = ernesto_k(u1, h, 0.1, 0.5)
    k_stab = k_a*res_orth_norm#/(euc_norm(restau0)+0.000000001)
    #k_stab = project(k_a, Qt)
    LPSl_stress = inner(0.05*k_stab*grad(tau),grad(Rt))*dx + inner(0.001*k_stab*div(tau),div(Rt))*dx  """
    ...
    while t < Tf + DOLFIN_EPS and iter < maxiter:
        ...
        Fu12 = dot(lhsFu12, v)*dx + \
               + inner(2.0*betav*Dincomp(U), Dincomp(v))*dx + (1.0/3)*inner(betav*div(U),div(v))*dx\
