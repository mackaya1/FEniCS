...
w_j = 2500.0                    # Characteristic Rotation Speed (rotations/s)
...
while j < loopend:
    ...
    mu_1 = 5.0*(4.0)*10E-3            # Solvent Viscosity 
    mu_2 = 5.0*(4.0)*10E-3            # Polymeric Viscosity
    ...
    Wet = Expression('We*0.5*(1.0+tanh(0.5*t-3.5))', t=0.0, We=Re, d=d, degree=d)
    ...
    # Stress Half Step
    a3 = (2.0*We/dt)*inner(tau,R)*dx + We*(inner(dot(u12,grad(tau)),R)*dx - inner(F12, R)*dx+inner(div(u12)*tau,R)*dx)#+0.0001*epstau*inner(grad(tau),grad(R))*dx
    L3 = (2.0*We/dt)*inner(tau0,R)*dx -inner(tau0,R)*dx + 2.0*(1.0-betav)*inner(sr0,R)*dx-(We*frac/(1-betav))*inner(tau0*tau0,R)#+0.0001*epstau*inner(grad(tau0),grad(R))*dx
    ...
    # Stress Full Step
    a4 = (1.0*We/dt)*inner(tau,R)*dx+inner(tau,R)*dx   #T0/T_0  
    L4 = (1.0*We/dt)*inner(tau0,R)*dx + 2.0*(1.0-betav)*inner(sr0,R)*dx- We*(inner(dot(u12,grad(tau12)),R)*dx - inner(Fr12, R)*dx + inner(div(u12)*tau12,R)*dx -(frac/(1-betav))*inner(tau12*tau12,R))
