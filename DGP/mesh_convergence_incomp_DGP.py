...
while j < loopend:
    ...
    while t < Tf + DOLFIN_EPS and iter < maxiter:
        ...
        lhsFus = ((u - u0)/dt + dot(u12, nabla_grad(u12)))
        Fus = dot(lhsFus, v)*dx + \
    ...
    if max(norm(w1.vector(),'linf'),norm(p1.vector(), 'linf')) < 10E6 and j==4 or j==1 or j==3:
        # Kinetic Energy
        plt.figure(0)
    ...
    """if max(norm(tau1_vec.vector(),'linf'),norm(w1.vector(), 'linf')) < 10E6 and j==4 or j==1:
