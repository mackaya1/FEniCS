...
while j < loopend:
    ...
    while t < Tf + DOLFIN_EPS and iter < maxiter:
        ...
        #Predictor Step [U*]
        lhsFus = ((u - u0)/dt + dot(u0, nabla_grad(U)))
        Fus = dot(lhsFus, v)*dx + inner(D-Dincomp(u),R)*dx  + \
              + inner(sigma(U, p0, tau0), Dincomp(v))*dx + Ra*Pr*inner(theta0*f,v)*dx \
              + 0.5*dot(p0*n, v)*ds - Pr*betav*(dot(nabla_grad(U)*n, v)*ds) \
    ...
    if max(norm(w1.vector(),'linf'),norm(p1.vector(), 'linf')) < 10E6 and abs(E_k) < 10 and j==1 or j==2 or j==4 or j==loopend:
